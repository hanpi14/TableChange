...
FROM (dw.dwd.dwd_ar_!{load_freq}i FOR SYSTEM_TIME AS OF '${snap_date}') A
         LEFT JOIN (dw.dwd.mid_product_business_mapping FOR SYSTEM_TIME AS OF '${snap_date}') B
                   ON A.product_name = B.product_name AND B.tag = 4
WHERE A.data_source = -11
  AND A.is_discard = 0
  AND A.is_split = 0
  AND A.is_ignore = 0
