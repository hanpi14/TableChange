DELETE FROM dw.dwd.dwd_ar_!{load_freq}i WHERE data_source=16;
...
FROM (SELECT R.*,
      ...
      FROM (SELECT O.*
            FROM (ods.sbt_prod.fnc_receivable_account FOR SYSTEM_TIME AS OF '${snap_date}') O
                     INNER JOIN (SELECT *
                                 FROM (ods.shebaotong.sbt_order FOR SYSTEM_TIME AS OF '${snap_date}') S
                                 WHERE DATE_FORMAT(S.create_time, 'yyyyMM') = S.area_order_month
                                   AND DATE_FORMAT(S.confirm_time, 'yyyyMM') = '${report_month}') P
            ...
            WHERE DATE_FORMAT(O.confirm_time, 'yyyyMM') <=
              ...
              AND O.source_no NOT IN (SELECT H.order_no
                                      FROM (ods.shebaotong.sbt_order FOR SYSTEM_TIME AS OF '${snap_date}') H
