...
SELECT IF(D.bill_no = L.order_no AND L.valid = 1, 10, 1)                                                AS data_source,
       ...
       CONCAT(IF(D.tax_report_rule = '2', '次月报税-抵扣-', '当月报税-抵扣-'), '',
              CASE A.item_code
                  ...
                  ELSE '其他' END)                                                                      AS order_item,
       IF(D.bill_no = L.order_no AND L.valid = 1, D.tax_rate,
          D.daishou_tax_rate)                                                                           AS order_tax_rate,
       D.tax_rate                                                                                       AS reward_tax_rate,
       0                                                                                                AS is_solo,
       1                                                                                                AS is_collect,
       IF((D.tax_report_rule = '2' AND D.tax_months < '${report_month}') OR
          (D.tax_report_rule = '1' AND DATE_FORMAT(D.create_time, 'yyyyMM') < DATE_FORMAT(
       ...
       ROUND(IF(A.item_code IN ('service', 'service_tax_fee'), A.diff_fee, 0),
             4)                                                                                         AS income_reward,
       ROUND(IF(A.item_code IN ('service', 'service_tax_fee'), A.diff_fee, 0),
             4)                                                                                         AS raw_income_reward,
       IF(K.tax_rule = 1, 2, 1)                                                                         AS tax_calc_mode,
       ROUND(IF(A.item_code IN ('service', 'service_tax_fee'), 0, A.diff_fee), 4)                       AS income_order,
       ROUND(IF(A.item_code IN ('service', 'service_tax_fee'), 0, A.diff_fee),
             4)                                                                                         AS raw_income_order,
       0                                                                                                AS income_overdue,
       ...
       ''                                                                                               AS raw_order_no,
       IFNULL(M.bill_no, REPLACE(UUID(),  '-',  ''))                                          AS batch_no
FROM (ods.sbt_prod.pay_payroll_refund FOR SYSTEM_TIME AS OF '${snap_date}') A
         INNER JOIN (ods.sbt_prod.pay_payroll_diff FOR SYSTEM_TIME AS OF '${snap_date}') B
                    ON A.diff_id
                        = B.id
         INNER JOIN (ods.sbt_prod.pay_payroll_emp FOR SYSTEM_TIME AS OF '${snap_date}') C
                    ON B.from_payroll_id
                        = C.payroll_id
         INNER JOIN (SELECT P.*, Q.confirm_time
                     FROM (ods.sbt_prod.pay_payroll_bill FOR SYSTEM_TIME AS OF '${snap_date}') P
                              INNER JOIN (SELECT O.bill_id, MIN(O.create_time) AS confirm_time
                                          FROM (ods.sbt_prod.pay_payroll_deal_record FOR SYSTEM_TIME AS OF '${snap_date}') O
                                          WHERE O.deal_status = '1'
                                            AND NOT EXISTS (SELECT 1
                                                            FROM (ods.sbt_prod.pay_payroll_deal_record FOR SYSTEM_TIME AS OF '${snap_date}') V
                                                            WHERE V.bill_id = O.bill_id
                                                              AND V.deal_status = '2')
                                          GROUP BY O.bill_id) Q ON P.bill_id = Q.bill_id
                     WHERE (
                             (P.tax_months = '${report_month}' AND P.tax_report_rule = '2' AND
                              DATE_FORMAT(Q.confirm_time, 'yyyyMM') <= '${report_month}')
                             OR
                             (P.tax_months < '${report_month}' AND P.tax_report_rule = '2' AND
                              DATE_FORMAT(Q.confirm_time, 'yyyyMM') = '${report_month}')
                             OR (P.tax_months =
                                 DATE_FORMAT(add_months(to_date(CONCAT('${report_month}', '01'), 'yyyyMMdd'),
                                                        -1),
                                             'yyyyMM') AND P.tax_report_rule = '1' AND
                                 DATE_FORMAT(Q.confirm_time, 'yyyyMM') <= '${report_month}')
                         )
                       AND P.alive_flag = '1'
         ...
         INNER JOIN (ods.sbt_prod.mbc_member_info FOR SYSTEM_TIME AS OF '${snap_date}') E ON A.member_id = E.member_id
         LEFT JOIN (ods.sbt_prod.oprt_boss_organization FOR SYSTEM_TIME AS OF '${snap_date}') F ON D.org_id = F.org_id
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '${snap_date}') G ON C.area_code = G.area_code
         LEFT JOIN (ods.sbt_prod.mbc_member_product FOR SYSTEM_TIME AS OF '${snap_date}') H ON H.m_pr_id = D.m_pr_id
         LEFT JOIN (ods.sbt_prod.basic_org_product FOR SYSTEM_TIME AS OF '${snap_date}') I ON I.pr_id = H.pr_id
         LEFT JOIN (ods.sbt_prod.supplier_info FOR SYSTEM_TIME AS OF '${snap_date}') J ON D.supplier_id = J.supplier_id
         LEFT JOIN (ods.sbt_prod.mbc_member_product_cover FOR SYSTEM_TIME AS OF '${snap_date}') K
         ...
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '${snap_date}') S ON D.area_code = S.area_code
         LEFT JOIN (ods.shebao_hro.hro_bill_constitute FOR SYSTEM_TIME AS OF '${snap_date}') L
                   ON D.bill_no = L.order_no AND L.valid = 1
         LEFT JOIN (ods.shebao_hro.hro_bill FOR SYSTEM_TIME AS OF '${snap_date}') M ON L.bill_id = M.id AND M.valid = 1
