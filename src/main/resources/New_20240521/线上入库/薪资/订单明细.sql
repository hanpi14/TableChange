...
SELECT B.data_source                                                                                    AS data_source,
       ...
       IF((B.tax_report_rule = '2' AND B.tax_months < '${report_month}') OR
          (B.tax_report_rule = '1' AND DATE_FORMAT(B.create_time, 'yyyyMM') < '${report_month}'), 1, 0) AS is_cross,
       0                                                                                                AS is_refund,
       0                                                                                                AS is_extra,
       0                                                                                                AS is_sup,
       ...
       ''                                                                                               AS comment,
       ''                                                                                               AS raw_order_no,
       IFNULL(B.hro_no, REPLACE(UUID(),  '-',  ''))                                           AS batch_no
FROM (ods.sbt_prod.pay_payroll_emp FOR SYSTEM_TIME AS OF '${snap_date}') A
         INNER JOIN (SELECT P.*,
                            Q.confirm_time,
                            T.bill_no                                         AS hro_no,
                            T.out_time,
                     ...
                     FROM (ods.sbt_prod.pay_payroll_bill FOR SYSTEM_TIME AS OF '${snap_date}') P
                              ...
                              LEFT JOIN (ods.shebao_hro.hro_bill_constitute FOR SYSTEM_TIME AS OF '${snap_date}') R
                                        ON P.bill_no = R.order_no
                                            AND R.valid = 1
                              LEFT JOIN (ods.shebao_hro.hro_bill FOR SYSTEM_TIME AS OF '${snap_date}') T
                                        ON R.bill_id = T.id
                                            AND T.valid = 1
                     WHERE (
                             ((
                                      ...
                                      OR (P.tax_months =
                                          DATE_FORMAT(
                                                  add_months(to_date(CONCAT('${report_month}', '01'), 'yyyyMMdd'),
                                                             -1),
                                      ...
                                      (
                                              (P.wirteoff_status = 6 OR (P.wirteoff_status = 7 AND P.receivable_status = 0)) AND (
                                                  (P.tax_months = '${report_month}' AND P.tax_report_rule = '2' AND
                                                   DATE_FORMAT(P.create_time, 'yyyyMM') <= '${report_month}')
                                                  OR (P.tax_months < '${report_month}' AND P.tax_report_rule = '2' AND
                                                      DATE_FORMAT(P.create_time, 'yyyyMM') = '${report_month}')
                                                  OR (P.tax_months =
                                                      DATE_FORMAT(
                                                              add_months(to_date(CONCAT('${report_month}', '01'), 'yyyyMMdd'),
                             ...
                             OR
                             (DATE_FORMAT(T.out_time, 'yyyyMM') = '${report_month}' AND R.bill_has = 1 AND T.state = 2)
                         )
                       AND P.alive_flag = '1'
         ...
         INNER JOIN (ods.sbt_prod.mbc_member_info FOR SYSTEM_TIME AS OF '${snap_date}') C ON A.member_id = C.member_id
         LEFT JOIN (ods.sbt_prod.oprt_boss_organization FOR SYSTEM_TIME AS OF '${snap_date}') D ON B.org_id = D.org_id
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '${snap_date}') E ON A.area_code = E.area_code
         LEFT JOIN (ods.sbt_prod.mbc_member_product FOR SYSTEM_TIME AS OF '${snap_date}') F ON B.m_pr_id = F.m_pr_id
         LEFT JOIN (ods.sbt_prod.basic_org_product FOR SYSTEM_TIME AS OF '${snap_date}') G ON F.pr_id = G.pr_id
         LEFT JOIN (ods.sbt_prod.supplier_info FOR SYSTEM_TIME AS OF '${snap_date}') H ON H.supplier_id = B.supplier_id
         LEFT JOIN (ods.sbt_prod.mbc_member_product_cover FOR SYSTEM_TIME AS OF '${snap_date}') I
         ...
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '${snap_date}') S ON B.area_code = S.area_code
-- LEFT JOIN (ods.shebao_hro.hro_bill_constitute FOR SYSTEM_TIME AS OF '${snap_date}' ) J ON B.bill_no=J.order_no AND J.valid=1
-- LEFT JOIN (ods.shebao_hro.hro_bill FOR SYSTEM_TIME AS OF '${snap_date}' ) K ON J.bill_id=K.id AND K.valid=1
