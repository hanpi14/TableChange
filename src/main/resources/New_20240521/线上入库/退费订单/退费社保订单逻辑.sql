...
FROM (ods.trade_center.trade_business_fee_refund_order FOR SYSTEM_TIME AS OF '${snap_date}') A
         INNER JOIN (ods.trade_center.trade_business_fee_refund_order_detail FOR SYSTEM_TIME AS OF '${snap_date}') B
                    ON A.id = B.order_id AND B.valid = 1
         INNER JOIN (ods.shebaotong.sbt_emp_diff FOR SYSTEM_TIME AS OF '${snap_date}') C
                    ON B.record_id = C.id
         INNER JOIN (ods.shebaotong.sbt_employee FOR SYSTEM_TIME AS OF '${snap_date}') D
                    ON C.emp_id = D.id
         LEFT JOIN (ods.sbt_prod.oprt_boss_organization FOR SYSTEM_TIME AS OF '${snap_date}') E
                   ON C.org_id = E.org_id
         INNER JOIN (ods.sbt_prod.mbc_member_info FOR SYSTEM_TIME AS OF '${snap_date}') F
                    ON C.member_id = F.member_id
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '${snap_date}') G
                   ON C.area_id = G.area_id
         LEFT JOIN (ods.sbt_prod.mbc_member_product FOR SYSTEM_TIME AS OF '${snap_date}') H
                   ON C.member_product_id = H.m_pr_id
         LEFT JOIN (ods.sbt_prod.basic_org_product FOR SYSTEM_TIME AS OF '${snap_date}') I
                   ON I.pr_id = H.pr_id
         LEFT JOIN (ods.shebaotong.sbt_operate_order_ins FOR SYSTEM_TIME AS OF '${snap_date}') J
                   ON C.operate_order_ins_id = J.id AND J.latest = 1
         LEFT JOIN (ods.shebaotong.sbt_operate_order FOR SYSTEM_TIME AS OF '${snap_date}') K
                   ON J.operate_order_id = K.id
         LEFT JOIN (ods.sbt_prod.supplier_info FOR SYSTEM_TIME AS OF '${snap_date}') L
                   ON K.supplier_id = L.supplier_id
         LEFT JOIN (ods.sbt_prod.sys_dictionary_dtl FOR SYSTEM_TIME AS OF '${snap_date}') M
                   ON C.service_type = M.dic_value AND M.dic_code = 'PR_SERVICE_TYPE'
         LEFT JOIN (ods.shebaotong.sbt_insurance_org FOR SYSTEM_TIME AS OF '${snap_date}') N
                   ON C.ins_org_id = N.id
         LEFT JOIN (ods.shebaotong.sbt_order_ins FOR SYSTEM_TIME AS OF '${snap_date}') O
         ...
         LEFT JOIN (ods.sbt_prod.fnc_receivable_account FOR SYSTEM_TIME AS OF '${snap_date}') P
                   ON A.receive_no = P.receive_no
         LEFT JOIN (ods.shebaotong.sbt_insurance_item FOR SYSTEM_TIME AS OF '${snap_date}') Q
                   ON C.ins_code = Q.ins_code
         LEFT JOIN (ods.sbt_prod.fnc_receivable_account FOR SYSTEM_TIME AS OF '${snap_date}') S
