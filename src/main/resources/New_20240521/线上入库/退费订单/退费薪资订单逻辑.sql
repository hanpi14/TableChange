...
FROM (ods.trade_center.trade_business_fee_refund_order FOR SYSTEM_TIME AS OF '${snap_date}') A
         ...
         INNER JOIN (ods.sbt_prod.pay_payroll_diff FOR SYSTEM_TIME AS OF '${snap_date}') C
                    ON B.record_id = C.id
         INNER JOIN (ods.sbt_prod.pay_payroll_emp FOR SYSTEM_TIME AS OF '${snap_date}') D
                    ON C.from_payroll_id = D.payroll_id
         INNER JOIN (ods.sbt_prod.pay_payroll_bill FOR SYSTEM_TIME AS OF '${snap_date}') E
         ...
         LEFT JOIN (ods.sbt_prod.oprt_boss_organization FOR SYSTEM_TIME AS OF '${snap_date}') G
                   ON E.org_id = G.org_id
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '${snap_date}') H
         ...
         LEFT JOIN (ods.sbt_prod.mbc_member_product_cover FOR SYSTEM_TIME AS OF '${snap_date}') L
         ...
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '${snap_date}') N
