...
INSERT INTO dw.dwd.dwd_order_emp_online_mi(data_source,
                                           order_no,
                                           order_type,
                                           business_type,
                                           ar_no,
...
SELECT IF(M.bill_no IS NULL, IF(O.order_no = A.order_no AND O.valid = 1, 10, 4), 9)                        AS data_source,
       ...
       IF(A.area_order_month < '202405', 1, 0)                                                             AS is_cross,
       0                                                                                                   AS is_refund,
       0                                                                                                   AS is_extra,
       0                                                                                                   AS is_sup,
...
FROM (ods.shebaotong.sbt_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') A
         INNER JOIN (SELECT SUM(IF(S.category = 3, R.org_fee, 0))                                AS corp_pension_amount,
                     ...
                     FROM (ods.shebaotong.sbt_order_ins FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') R
                              INNER JOIN (ods.shebaotong.sbt_insurance_item FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') S
                                         ON R.ins_code = S.ins_code
                              INNER JOIN (ods.shebaotong.sbt_insurance_org FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') T
                                         ON R.ins_org_id = T.id
                              LEFT JOIN (ods.shebaotong.sbt_operate_order_ins FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') V
                                        ON R.id = V.order_ins_id AND V.latest = 1
                              LEFT JOIN (ods.shebaotong.sbt_operate_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') U
                    ...
                    ON A.id = B.order_id
                        AND IF(B.order_state = 8 AND A.receive_no NOT IN (SELECT Q.receive_no
                                                                          FROM (ods.sbt_prod.fnc_receivable_account FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') Q
                                                                          WHERE Q.alive_flag = '1'
                                                                            AND Q.confirm_status = '1') AND
                               A.order_type = 1, 0, 1) = 1
         INNER JOIN (ods.shebaotong.sbt_employee FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') C
                    ON B.emp_id = C.id
         LEFT JOIN (ods.sbt_prod.oprt_boss_organization FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') D
                   ON A.org_id = D.org_id
         INNER JOIN (ods.sbt_prod.mbc_member_info FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') E
                    ON A.member_id = E.member_id
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') F
                   ON A.area_id = F.area_id
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') G
                   ON B.area_id = G.area_id
         LEFT JOIN (ods.sbt_prod.mbc_member_product FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') I
                   ON A.member_product_id = I.m_pr_id
         LEFT JOIN (ods.sbt_prod.basic_org_product FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') J
                   ON J.pr_id = I.pr_id
         LEFT JOIN (ods.sbt_prod.supplier_info FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') K
         ...
         LEFT JOIN (ods.shebaotong.sbt_bill_dtl FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') L
                   ON A.id = L.order_id AND L.data_type = 1 AND L.alive_flag = 1
         LEFT JOIN (ods.shebaotong.sbt_bill FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') M
                   ON L.bill_id = M.id
         LEFT JOIN (ods.sbt_prod.fnc_receivable_account FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') H
                   ON IF(A.receive_no = '', M.receive_no, A.receive_no) = H.receive_no
         LEFT JOIN (ods.shebao_hro.hro_bill_constitute FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') O
                   ON O.order_no = A.order_no AND O.valid = 1
         LEFT JOIN (ods.shebao_hro.hro_bill FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') P
                   ON O.bill_id = P.id AND P.valid = 1
         LEFT JOIN (SELECT MAX(create_time) create_time, order_id
                    FROM (ods.shebaotong.sbt_order_record FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00')
                    WHERE oprt_node = 15
                    GROUP BY order_id) Q ON Q.order_id = A.id
         LEFT JOIN (SELECT order_id, sipr_tax_rate, tax_rate
                    FROM (ods.shebaotong.sbt_order_subject FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00')
                    WHERE fee_type = 1
                    GROUP BY order_id) R ON R.order_id = A.id AND A.ods_sbt_order_20240411_id = 0
         LEFT JOIN (ods.sbt_prod.ec_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') S
                   ON A.ods_sbt_order_20240411_id = S.order_id
         LEFT JOIN (ods.sbt_prod.ec_order_second_socialins FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') U
                   ON S.order_id = U.order_id AND U.emp_id = C.sbt_emp_id AND
                      IF(U.insurance_type = 'shebao', 1, 2) = B.ins_type AND
                      U.account_status = 'keep'
         LEFT JOIN (ods.sbt_prod.supplier_info FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') V
...
WHERE (
        (A.confirm_flag = '1' AND
         ...
         (
                 (A.area_order_month = '202405' AND
                  DATE_FORMAT(IF(Q.order_id = A.id, Q.create_time, A.confirm_time), 'yyyyMM') <= '202405')
                 OR
                 (A.area_order_month <= '202405' AND
                  DATE_FORMAT(IF(Q.order_id = A.id, Q.create_time, A.confirm_time), 'yyyyMM') = '202405')
        ...
        (M.bill_state = 2 AND A.order_type = 2 AND A.ods_sbt_order_20240411_id = 0 AND
         (
                 (M.bill_month = '202405' AND
                  IF(A.ods_sbt_order_20240411_id != 0, DATE_FORMAT(A.create_time, 'yyyyMM') <= '202405',
                     DATE_FORMAT(L.create_time, 'yyyyMM') <= '202405') OR
                  DATE_FORMAT(M.bill_time, 'yyyyMM') <= '202405')
                 OR
                 (M.bill_month <= '202405' AND
                  IF(A.ods_sbt_order_20240411_id != 0, DATE_FORMAT(A.create_time, 'yyyyMM') = '202405',
                     DATE_FORMAT(L.create_time, 'yyyyMM') = '202405') OR DATE_FORMAT(M.bill_time, 'yyyyMM') = '202405')
             )
            )
        OR
        (
                    A.confirm_flag = '1' AND DATE_FORMAT(P.out_time, 'yyyyMM') = '202405' AND O.bill_has = 1
            )
        OR
        (
                    M.bill_state = 2 AND A.order_type = 2 AND A.ods_sbt_order_20240411_id != 0 AND
                    (M.bill_month = '202405' OR DATE_FORMAT(M.bill_time, 'yyyyMM') = '202405')
            )
    )
  AND A.order_no NOT IN (SELECT T.order_no
                         FROM (ods.shebaotong.sbt_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') T
                         WHERE T.order_state = 5
                           AND T.order_type = 2
                           AND T.id != L.order_id)
  AND A.order_no NOT IN (SELECT W.order_no
                         FROM (ods.shebaotong.sbt_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') W
                         WHERE A.order_state = 5
                           AND DATE_FORMAT(H.confirm_time, 'yyyyMM') != DATE_FORMAT(Q.create_time, 'yyyyMM')
                           AND A.order_type = 1
                           AND A.area_order_month != '202405')
  AND A.order_no NOT IN (SELECT X.order_no
                         FROM (ods.shebaotong.sbt_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') X
                         WHERE A.order_state = 5
                           AND DATE_FORMAT(H.confirm_time, 'yyyyMM') != DATE_FORMAT(Q.create_time, 'yyyyMM')
                           AND A.order_type = 2
                           AND A.area_order_month != '202405'
                           AND M.bill_month != '202405'
                           AND DATE_FORMAT(M.bill_time, 'yyyyMM') != '202405');
