...
SELECT -1                                                                                               AS data_source,
       ...
       IFNULL(A.order_month, '202405')                                                                  AS raw_order_month,
       IFNULL(DATE_FORMAT(CONCAT(B.tax_month, '-01'), 'yyyyMM'), '202405')                              AS fee_month,
...
FROM (SELECT O.id,
             ...
             O.create_time,
             O.memo
      FROM (ods.trade_center.trade_business_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') O
      WHERE O.create_order = 1
        ...
        AND ((DATE_FORMAT(IF(O.order_month = '', IF(O.order_date = '', O.order_start_date, O.order_date),
                             CONCAT(O.order_month, '-01')), 'yyyyMM') <= '202405'
          AND DATE_FORMAT(O.confirm_time, 'yyyyMM') = '202405') OR
             (DATE_FORMAT(IF(O.order_month = '', IF(O.order_date = '', O.order_start_date, O.order_date),
                             CONCAT(O.order_month, '-01')), 'yyyyMM') = '202405'
                 AND DATE_FORMAT(O.confirm_time, 'yyyyMM') <= '202405'))
        AND O.test_flag = 0
        AND O.service_type = 1
        AND O.audit_state = 3
        AND O.order_state != 5
        AND O.memo NOT LIKE '%补录%') A
         INNER JOIN (ods.trade_center.trade_business_order_xzpeople FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') B
                    ON A.id = B.order_id AND B.valid = 1
         LEFT JOIN (ods.sbt_prod.mbc_member_product FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') C
                   ON A.member_product_id = C.m_pr_id
         LEFT JOIN (ods.sbt_prod.basic_org_product FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') D ON C.pr_id = D.pr_id
         LEFT JOIN (ods.sbt_prod.oprt_boss_organization FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') E
                   ON E.org_id = A.contract_company_id
         LEFT JOIN (ods.sbt_prod.supplier_info FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') F
                   ON B.supplier_id = F.supplier_id
         INNER JOIN (ods.sbt_prod.mbc_member_info FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') G ON A.member_id = G.member_id
         LEFT JOIN (ods.sbt_prod.sys_dictionary_dtl FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') N
                   ON D.service_type = N.dic_value AND N.dic_code = 'PR_SERVICE_TYPE'
         LEFT JOIN (ods.sbt_prod.basic_ins_area FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') S ON A.city_code = S.area_code;
...
SELECT -1                                                                                               AS data_source,
       ...
       '202405'                                                                                         AS order_month,
       IFNULL(A.order_month, '202405')                                                                  AS raw_order_month,
       IFNULL(A.fee_month, '202405')                                                                    AS fee_month,
       G.member_code                                                                                    AS member_code,
       G.member_name                                                                                    AS member_name,
       E.org_name                                                                                       AS party_b,
...
0))                          AS income_reward_type_8
      FROM (ods.trade_center.trade_business_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') O
               INNER JOIN (ods.trade_center.trade_business_order_detail FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') P
...
SELECT -1                                                                                               AS data_source,
       ...
       '202405'                                                                                         AS order_month,
       IFNULL(A.order_month, '202405')                                                                  AS raw_order_month,
       IFNULL(A.order_month, '202405')                                                                  AS fee_month,
       G.member_code                                                                                    AS member_code,
       G.member_name                                                                                    AS member_name,
       E.org_name                                                                                       AS party_b,
       A.emp_name                                                                                       AS emp_name,
...
FROM (SELECT O.order_code                                                           AS order_no,
      ...
      FROM (ods.trade_center.trade_business_order FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') O
               INNER JOIN (ods.trade_center.trade_business_order_xmpeople FOR SYSTEM_TIME AS OF '2024-05-18 03:00:00') P
                          ON O.id = P.order_id
                              AND P.valid = 1
      WHERE O.create_order = 1
        AND (O.order_month != '' OR O.order_date != '' OR O.order_start_date != '')
