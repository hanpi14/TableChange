...
SELECT CASE
       ...
       IF(A.order_month < '202205', 1, 0) AS is_cross,
...
FROM (
         ...
         FROM (
                  ...
                  FROM ods_fnc_receivable_account_20230113 O
                           LEFT JOIN ods_pay_payroll_bill_20230113 P ON O.order_no = P.bill_no AND P.wirteoff_status=6 AND P.alive_flag=1
                  WHERE (
                          (O.order_month = '202205' AND DATE_FORMAT(IF(O.order_no = P.bill_no AND P.wirteoff_status=6 AND P.alive_flag=1,O.create_time,O.confirm_time), '%Y%m') <= '202205') OR
                          (O.order_month < '202205'
                              AND DATE_FORMAT(IF(O.order_no = P.bill_no AND P.wirteoff_status=6 AND P.alive_flag=1,O.create_time,O.confirm_time), '%Y%m') = '202205'
                              AND (O.note IS NULL
                                  OR (O.note NOT LIKE '%补录%'
                                      AND O.note NOT LIKE '%补数据%')))
                      )
                    AND DATE_FORMAT(O.create_time, '%Y%m') <= '202205'
