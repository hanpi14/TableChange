...
SELECT 16 AS data_source,
       ...
       IF(A.order_month < '202301', 1, 0) AS is_cross,
...
FROM (
         ...
         FROM (
                  ...
                  SELECT O.* FROM ods_fnc_receivable_account_20230112 O
                                      INNER JOIN (SELECT * FROM ods_sbt_order_20230112 S WHERE DATE_FORMAT(S.create_time, '%Y%m')=S.area_order_month
                                                                                           AND DATE_FORMAT(S.confirm_time, '%Y%m')='202301')  P
                                                 ON O.order_no=P.order_no  AND O.alive_flag=1
                  WHERE  DATE_FORMAT(O.confirm_time, '%Y%m')<=DATE_FORMAT(DATE_SUB(P.confirm_time,INTERVAL 1 MONTH), '%Y%m')
                    AND DATE_FORMAT(O.create_time, '%Y%m') <= '202301'
                    AND O.source_no NOT IN (
                      SELECT H.order_no
                      FROM ods_sbt_order_20230112 H
                      WHERE H.confirm_flag = '0'
                  )
              ) R
                  LEFT JOIN
              ods_mbc_member_product_20230112 S ON R.m_pr_id = S.m_pr_id
                  LEFT JOIN ods_basic_org_product_20230112 T ON S.pr_id = T.pr_id
                  LEFT JOIN ods_sys_dictionary_dtl_20230112 U
                            ON T.service_type = U.dic_value AND U.dic_code = 'PR_SERVICE_TYPE'
                  LEFT JOIN ods_sys_dictionary_dtl_20230112 V
                            ON T.service_code = V.dic_value AND V.dic_code = 'PR_SERVICE_CODE'
                  LEFT JOIN ods_mbc_member_info_20230112 W ON R.member_id = W.member_id
                  LEFT JOIN ods_oprt_boss_organization_20230112 X ON R.org_id = X.org_id
                  LEFT JOIN ods_basic_ins_area_20230112 Y ON R.area_code = Y.area_code
                  LEFT JOIN ods_oprt_bill_manual_handle_20230112 Z ON R.receive_id = Z.receive_id
                  LEFT JOIN ods_fnc_receivable_account_20230112 N ON R.parent_id = N.receive_id
