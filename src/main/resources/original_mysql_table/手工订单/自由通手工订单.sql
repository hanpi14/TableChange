...
SELECT -1                                                                        AS data_source,
       ...
       B.service_amount / 10000                                                  AS income_reward,
       B.service_amount / 10000                                                  AS raw_income_reward,
       IF(C.single_flag = 1, 0, B.paid_amount / 10000)                           AS income_order,
       B.paid_amount / 10000                                                     AS raw_income_order,
       0                                                                         AS income_overdue,
       ...
       B.service_rate / 100                                                      AS order_parm,
       0                                                                         AS indv_net_pay,
       ...
       AES_DECRYPT(FROM_BASE64(B.account_no), LEFT(UNHEX(SHA1('bCdEfGhI')), 16)) AS debit_card_no,
       bank                                                                      AS bank_name,
       ''                                                                        AS bank_region,
       ''                                                                        AS tax_corp,
...
FROM (SELECT O.id,
      ...
      WHERE O.create_order = 1
        ...
        AND O.memo NOT LIKE '%补录%') A
         INNER JOIN ods_trade_business_order_dmqtpeople_20240410 B ON A.id = B.order_id AND B.valid = 1
...
FROM (SELECT O.id,
      ...
      WHERE O.create_order = 1
        ...
        AND O.order_state != 5
        AND O.memo NOT LIKE '%补录%') A
         INNER JOIN ods_trade_business_order_jszbpeople_20240410 B ON A.id = B.order_id AND B.valid = 1
         LEFT JOIN ods_mbc_member_product_20240410 C ON A.member_product_id = C.m_pr_id
         LEFT JOIN ods_basic_org_product_20240410 D ON C.pr_id = D.pr_id
         LEFT JOIN ods_oprt_boss_organization_20240410 E ON E.org_id = A.contract_company_id
         INNER JOIN ods_mbc_member_info_20240410 G ON A.member_id = G.member_id;
