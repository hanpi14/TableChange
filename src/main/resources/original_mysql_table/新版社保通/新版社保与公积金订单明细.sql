...
SELECT IF(M.bill_no IS NULL, IF(O.order_no = A.order_no AND O.valid = 1, 10, 4), 9)                     AS data_source,
       ...
       AES_DECRYPT(FROM_BASE64(IFNULL(C.mobile_phone, '')), LEFT(UNHEX(SHA1('AbCdEfGh')), 16))          AS mobile,
       CASE
       ...
       IF(A.account_property = 2, 0, B.income_indv_order + B.income_corp_order + B.income_late_order)  AS income_order,
...
FROM ods_sbt_order_20240411 A
         INNER JOIN (SELECT SUM(IF(S.category = 3, R.org_fee, 0))                                AS corp_pension_amount,
                     ...
                     FROM ods_sbt_order_ins_20240411 R
                              ...
                              LEFT JOIN ods_sbt_operate_order_ins_20240411 V
                    ...
                    ON A.id = B.order_id
                        AND IF(B.order_state = 8 AND A.receive_no NOT IN (SELECT Q.receive_no
                                                                          FROM ods_fnc_receivable_account_20240411 Q
