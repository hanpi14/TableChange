...
FROM ods_sbt_order_20240411 A #社保订单
         INNER JOIN ods_sbt_order_add_fee_20240411 B #订单补充费用
                    ON A.id = B.order_id
         LEFT JOIN ods_oprt_boss_organization_20240411 D #BOSS组织机构
ON A.org_id = D.org_id
    INNER JOIN ods_mbc_member_info_20240411 E #会员信息
             ON A.member_id = E.member_id
         LEFT JOIN ods_basic_ins_area_20240411 F #地区设置
