...
SELECT -12                                                                                              AS data_source,
       ...
       IF(F.virtual_flag = 0, CASE I.open_invoice_type
                                  WHEN 1 THEN 8
                                  WHEN 2 THEN IF(M.dic_value = '2' OR M.dic_value = '3', 2, 1) END, -1) AS business_type,
       A.receive_no                                                                                     AS ar_no,
       '202312'                                                                                         AS order_month,
       ...
       AES_DECRYPT(FROM_BASE64(D.idcard_no),
           LEFT(UNHEX(SHA1('ABCDEFGH')), 16))                                                   AS emp_id_no,
       AES_DECRYPT(FROM_BASE64(IFNULL(D.mobile_phone, '')),
           LEFT(UNHEX(SHA1('ABCDEFGH')), 16))                                AS mobile,
       CASE
       ...
       IF(C.area_order_month < '202310', 1, 0)                                       AS is_cross,
...
FROM ods_trade_business_fee_refund_order_20240411 A
         INNER JOIN ods_trade_business_fee_refund_order_detail_20240411 B
ON A.id = B.order_id AND B.valid = 1
    INNER JOIN ods_sbt_emp_diff_20240411 C
    ON B.record_id = C.id
    INNER JOIN ods_sbt_employee_20240411 D
    ON C.emp_id = D.id
    LEFT JOIN ods_oprt_boss_organization_20240411 E
    ON C.org_id = E.org_id
    INNER JOIN ods_mbc_member_info_20240411 F
    ON C.member_id = F.member_id
    LEFT JOIN ods_basic_ins_area_20240411 G
    ON C.area_id = G.area_id
    LEFT JOIN ods_mbc_member_product_20240411 H
    ON C.member_product_id = H.m_pr_id
    LEFT JOIN ods_basic_org_product_20240411 I
    ON I.pr_id = H.pr_id
    LEFT JOIN ods_sbt_operate_order_ins_20240411 J
    ON C.operate_order_ins_id = J.id AND J.latest = 1
    LEFT JOIN ods_sbt_operate_order_20240411 K
    ON J.operate_order_id = K.id
    LEFT JOIN ods_supplier_info_20240411 L
    ON K.supplier_id = L.supplier_id
    LEFT JOIN ods_sys_dictionary_dtl_20240411 M
    ON C.service_type = M.dic_value AND M.dic_code = 'PR_SERVICE_TYPE'
    LEFT JOIN ods_sbt_insurance_org_20240411 N
    ON C.ins_org_id = N.id
    LEFT JOIN ods_sbt_order_ins_20240411 O
    ...
    LEFT JOIN ods_fnc_receivable_account_20240411 P
ON A.receive_no = P.receive_no
         LEFT JOIN ods_sbt_insurance_item_20240411 Q
             ON C.ins_code = Q.ins_code
         LEFT JOIN ods_fnc_receivable_account_20240411 S
ON A.receive_no = S.receive_no
WHERE (
    (S.order_month = '202312' AND DATE_FORMAT(S.confirm_time, '%Y%m') <= '202312')
        OR (S.order_month <= '202312' AND DATE_FORMAT(S.confirm_time, '%Y%m') = '202312')
    )
