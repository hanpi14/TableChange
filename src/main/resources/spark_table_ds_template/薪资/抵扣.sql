...
DATE_SUB(to_date(CONCAT('${report_month}', '01'), 'yyyyMMdd'), -1),
...
FROM ods.sbt_prod.pay_payroll_refund A
INNER JOIN ods.sbt_prod.pay_payroll_diff B
ON A.diff_id
= B.id
INNER JOIN ods.sbt_prod.pay_payroll_emp C
ON B.from_payroll_id
= C.payroll_id
INNER JOIN (SELECT P.*, Q.confirm_time
FROM ods.sbt_prod.pay_payroll_bill P
INNER JOIN (SELECT O.bill_id, MIN(O.create_time) AS confirm_time
FROM ods.sbt_prod.pay_payroll_deal_record O
WHERE O.deal_status = '1'
AND NOT EXISTS(SELECT 1
FROM ods.sbt_prod.pay_payroll_deal_record V
...
INNER JOIN ods.sbt_prod.mbc_member_info E ON A.member_id = E.member_id
LEFT JOIN ods.sbt_prod.oprt_boss_organization F ON D.org_id = F.org_id
LEFT JOIN ods.sbt_prod.basic_ins_area G ON C.area_code = G.area_code
LEFT JOIN ods.sbt_prod.mbc_member_product H ON H.m_pr_id = D.m_pr_id
LEFT JOIN ods.sbt_prod.basic_org_product I ON I.pr_id = H.pr_id
LEFT JOIN ods.sbt_prod.supplier_info J ON D.supplier_id = J.supplier_id
LEFT JOIN ods.sbt_prod.mbc_member_product_cover K
ON H.m_pr_id = K.m_pr_id AND D.area_code = K.area_code AND K.alive_flag = '1'
LEFT JOIN ods.sbt_prod.sys_dictionary_dtl N
ON I.service_type = N.dic_value AND N.dic_code = 'PR_SERVICE_TYPE'
LEFT JOIN ods.sbt_prod.basic_ins_area S ON D.area_code = S.area_code
LEFT JOIN ods.shebao_hro.hro_bill_constitute L ON D.bill_no = L.order_no AND L.valid = 1
LEFT JOIN ods.shebao_hro.hro_bill M ON L.bill_id = M.id AND M.valid = 1
