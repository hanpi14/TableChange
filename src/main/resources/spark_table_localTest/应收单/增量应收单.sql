...
FROM (SELECT R.*,
      ...
      FROM (SELECT O.*
            FROM ods.sbt_prod.fnc_receivable_account O
                     LEFT JOIN ods.sbt_prod.pay_payroll_bill P
                               ON O.order_no = P.bill_no AND P.wirteoff_status = 6 AND P.alive_flag = 1
            WHERE (
                (O.order_month = '202405' AND DATE_FORMAT(IF(O.order_no = P.bill_no AND P.wirteoff_status = 6 AND
                                                             P.alive_flag = 1, O.create_time, O.confirm_time),
                                                          'yyyyMM') <= '202405') OR
                (O.order_month < '202405'
                    AND DATE_FORMAT(
                                IF(O.order_no = P.bill_no AND P.wirteoff_status = 6 AND P.alive_flag = 1, O.create_time,
                                   O.confirm_time), 'yyyyMM') = '202405'
                    AND (O.note IS NULL
