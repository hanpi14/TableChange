...
SELECT IF(D.bill_no = L.order_no AND L.valid = 1, 10, 1)                                                AS data_source,
       ...
       0                                                                                                AS is_solo,
       1                                                                                                AS is_collect,
       IF((D.tax_report_rule = '2' AND D.tax_months < '202405') OR
          (D.tax_report_rule = '1' AND DATE_FORMAT(D.create_time, 'yyyyMM') < DATE_FORMAT(
                  DATE_SUB(to_date(CONCAT('202405', '01'), 'yyyyMMdd'), -1),
...
FROM ods.sbt_prod.pay_payroll_refund A
         ...
         INNER JOIN (SELECT P.*, Q.confirm_time
                     ...
                     WHERE (
                             (P.tax_months = '202405' AND P.tax_report_rule = '2' AND
                              DATE_FORMAT(Q.confirm_time, 'yyyyMM') <= '202405')
                             OR
                             (P.tax_months < '202405' AND P.tax_report_rule = '2' AND
                              DATE_FORMAT(Q.confirm_time, 'yyyyMM') = '202405')
                             OR (P.tax_months =
