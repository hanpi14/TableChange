...
SELECT B.data_source                                                                                  AS data_source,
       ...
       IF((B.tax_report_rule = '2' AND B.tax_months < '202405') OR
          (B.tax_report_rule = '1' AND DATE_FORMAT(B.create_time, 'yyyyMM') < '202405'), 1, 0)        AS is_cross,
       0                                                                                              AS is_refund,
       0                                                                                              AS is_extra,
       0                                                                                              AS is_sup,
...
FROM ods.sbt_prod.pay_payroll_emp A
         INNER JOIN (SELECT P.*,
                            Q.confirm_time,
                            T.bill_no                                         AS hro_no,
                            T.out_time,
                     ...
                     FROM ods.sbt_prod.pay_payroll_bill P
                              ...
                              LEFT JOIN ods.shebao_hro.hro_bill_constitute R ON P.bill_no = R.order_no
                         AND R.valid = 1
                              LEFT JOIN ods.shebao_hro.hro_bill T ON R.bill_id = T.id
                         AND T.valid = 1
                     WHERE (
                             ((
                                      ...
                                      OR (P.tax_months =
                                          DATE_FORMAT(
                                                  add_months(TO_DATE(CONCAT('202405', '01'), 'yyyyMMdd'), -1),
                                      ...
                                      (
                                              (P.wirteoff_status = 6 OR (P.wirteoff_status = 7 AND P.receivable_status = 0)) AND (
                                                  (P.tax_months = '202405' AND P.tax_report_rule = '2' AND
                                                   DATE_FORMAT(P.create_time, 'yyyyMM') <= '202405')
                                                  OR (P.tax_months < '202405' AND P.tax_report_rule = '2' AND
                                                      DATE_FORMAT(P.create_time, 'yyyyMM') = '202405')
                                                  OR (P.tax_months =
                                                      DATE_FORMAT(
                                                              add_months(TO_DATE(CONCAT('202405', '01'), 'yyyyMMdd'), -1),
                             ...
                             OR (DATE_FORMAT(T.out_time, 'yyyyMM') = '202405' AND R.bill_has = 1 AND T.state = 2)
                         )
                       AND P.alive_flag = '1'
         ...
         INNER JOIN ods.sbt_prod.mbc_member_info C ON A.member_id = C.member_id
         LEFT JOIN ods.sbt_prod.oprt_boss_organization D ON B.org_id = D.org_id
         LEFT JOIN ods.sbt_prod.basic_ins_area E ON A.area_code = E.area_code
         LEFT JOIN ods.sbt_prod.mbc_member_product F ON B.m_pr_id = F.m_pr_id
         LEFT JOIN ods.sbt_prod.basic_org_product G ON F.pr_id = G.pr_id
         LEFT JOIN ods.sbt_prod.supplier_info H ON H.supplier_id = B.supplier_id
         LEFT JOIN ods.sbt_prod.mbc_member_product_cover I
         ...
         LEFT JOIN ods.sbt_prod.basic_ins_area S ON B.area_code = S.area_code
-- LEFT JOIN ods.shebao_hro.hro_bill_constitute J ON B.bill_no=J.order_no AND J.valid=1
-- LEFT JOIN ods.shebao_hro.hro_bill K ON J.bill_id=K.id AND K.valid=1
