...
FROM ods.trade_center.trade_business_fee_refund_order A
         ...
         LEFT JOIN (SELECT T.order_id      AS order_id,
                           T.sipr_tax_rate AS sipr_tax_rate,
                           T.tax_rate      AS tax_rate
                    FROM (SELECT order_id,
                                 sipr_tax_rate,
                                 tax_rate,
                                 ROW_NUMBER() OVER (PARTITION BY order_id ORDER BY order_id) AS rt
                          FROM ods.shebaotong.sbt_order_subject
                          WHERE fee_type = 1) T
                    WHERE T.rt = 1) R ON R.order_id = O.order_id
